...
var Topic = function(){
    this.turns=[];
    this.ID=topics.topics.length;    
    topics.topics.push(this);
    this.maxNegativeTopicDist = 0;
    this.maxPositiveTopicDist = 0;
    this.offsetX = 0;
    controller.updateTopicsOffsetX();
}
...
screen.update=function(e){
    ...
    screen.offsetY = Math.min(0,nElemsY-1-controller.getCurrentTurn())*screen.template.elem.height;
...
screen.drawHeader=function(topicsList,turn){
 screen.context.save();
    ...
    while(offsetX < screen.template.x+screen.template.width && iTopic<topicsList.length){
        maxI =controller.getTopicSpan(topicsList[iTopic])+2;
...
screen.drawTurns=function(topicsList,turn){
    ...
    var minutes = 0;
    var seconds = 0;
    var turnCounter =controller.getCurrentTurn()-1;
    var turnI =controller.getCurrentTurn()+Math.floor(screen.offsetY/screen.template.elem.height);
    ...
    while(/*offsetX < screen.template.x+screen.template.width &&*/ iTopic<topicsList.length){
       ...
       screen.context.fillText(turnsList[iTurn].ID,screen.rightBorder.x+5,p2.y);
        ...
        offsetX += (controller.getTopicSpan(topicsList[iTopic])+2)*screen.template.elem.width;
        iTopic++;
    }
...
screen.draw=function(){
    ...
    //drawCurrent
    screen.drawTopics(topics.topics,controller.getCurrentTurn());
    screen.drawTurns(topics.topics,controller.getCurrentTurn());
    screen.drawHeader(topics.topics,controller.getCurrentTurn());
...
ki.loop=function(){
    ...
    if(ki.running){
      ...
      if (ki.status == ki.Type.Shift){
          ...
          if(topics.preTurn==null){
             controller.turnFrom(topics.topics[0],topics.topics[0],
                                        0,
                                        0,
                                        topics.overallTime); 
    }
         ...
         if (pX<=cShift){
            controller.turnFrom(topics.preTurn.topic,topics.preTurn.topic,
                 topics.preTurn.topicDist,
                 topics.preTurn.topicDist+1,
                 topics.overallTime); 
            ki.status=ki.Type.Shift; 

        } else if(pX<=cNew){
          var topic= new Topic();
          controller.turnFrom(topic,topic,
                                      0,
                                      0,
                                      topics.overallTime); 
          ki.status=ki.Type.New; 
          ...
          var rIdxTopic = Math.min(Math.floor(Math.random()*topics.topics.length),topics.topics.length);
          if(topics.topics[rIdxTopic].turns.length==0){
                controller.turnFrom(topics.topics[rIdxTopic],topics.topics[rIdxTopic],
                                        0,
                                        0,
                                        topics.overallTime); 
          }
          var rTurn  = topics.topics[rIdxTopic].turns[topics.topics[rIdxTopic].turns.length-1];
          controller.turnFrom(rTurn.topic,rTurn.topic,
                                        rTurn.topicDist,
                                        rTurn.topicDist,
                                        topics.overallTime); 
          ...
          var rIdxTopic = Math.floor(Math.random()*topics.topics.length);
          if(topics.topics[rIdxTopic].turns.length==0){
                controller.turnFrom(topics.topics[rIdxTopic],topics.topics[rIdxTopic],
                                        0,
                                        0,
          ...
          var rTurn  = topics.topics[rIdxTopic].turns[topics.topics[rIdxTopic].turns.length-1];
          controller.turnFrom(rTurn.topic,topics.preTurn.topic,
                                        topics.preTurn.topicDist,
                                        rTurn.topicDist,
                                        topics.overallTime); 
      ...
      var nElemsY =  Math.floor(screen.template.height/screen.template.elem.height);
      screen.offsetY = Math.min(0,nElemsY-1-controller.getCurrentTurn())*screen.template.elem.height;
      screen.offsetX = (-topics.preTurn.topic.offsetX+topics.preTurn.topic.maxNegativeTopicDist-topics.preTurn.topicDist-2)*screen.template.elem.width+screen.template.width/2;
      screen.draw();
...
controller.replay=function(){
    ...
    if (!controller.play){
        ...
        var newTime =$.now();
        topics.lastTime= newTime;
   }
    
}
controller.initButtons=function(){
        $('#btnDownload').click(function(){
            var json = JSON.stringify(JSON.decycle(topics));
            var data = "text/json;charset=utf-8," + encodeURIComponent(json);
            
            $("#json_download").attr('href','data:' + data);
            document.getElementById('json_download').click();
            //topics=JSON.retrocycle(JSON.parse(JSON.stringify(JSON.decycle(topics))));
        ...
        $('#btnDownloadCSV').click(function(){
            ...
            var turn = topics.preTurn;
            var list = [];
            for (var idx = controller.getCurrentTurn()-1;idx>0;idx--){
                   list.push([turn.ID,turn.topic.ID,turn.preTopic.ID,turn.topicDist,turn.preTopicDist]);
                   turn = turn.preTurn;
            }
            ...
            for(var idx=0; idx<list.length; idx++){
                    csvRows.push(list[idx].join('&'));   
            }
            var csvStr= csvRows.join('\n');
            
            
            csvStr='turn&topic&pretopic&topicdistance&pretopicdistance\n'+csvStr;
            

            var data = "attachment/csv," + encodeURIComponent(csvStr);
            
            $("#csv_download").attr('href','data:' + data);
        ...
        function handleFileSelect(evt) {
            ...
            reader.onload = (function(theFile) {
                return function(e) {
                    topics=JSON.retrocycle(JSON.parse(e.target.result));
                               screen.update();
                    screen.draw();
                    controller.pause();
                    ki.stop();
        ...
        $('#btnSave').click(function(){
            ...
            $('#display').height((controller.getCurrentTurn()+3)*screen.refSize);
        ...
        $('#btnUndo').click(function(){
            ...
            controller.shifting=false;
            var turn = controller.popTurn();
            var nElemsY =  Math.floor(screen.template.height/screen.template.elem.height);
            screen.offsetY = Math.min(0,nElemsY-controller.getCurrentTurn()-1)*screen.template.elem.height;
            screen.draw();
        });
        ...
        $('#btnBackward').click(function(){
            topics.overallTime=Math.max(topics.overallTime-1000,0);
        });
        $('#btnBackward').dblclick(function(e){
            e.preventDefault();
            topics.overallTime=Math.max(topics.overallTime-58000,0);
        });
        $('#btnForward').click(function(){
             topics.overallTime+=1000;
        });
        $('#btnForward').dblclick(function(e){
             e.preventDefault();
             topics.overallTime+=58000;
        });
        ...
        $('#btnUp').click(function(){
            controller.shifting=false;
            //if(screen.template.height-screen.template.elem.height*(controller.turnCounter+1)<0){
                screen.offsetY-=screen.template.elem.height;
                screen.draw();
            //}
            controller.pause();
            ki.stop();
            //debug_log3("btnUP clicked");
        });
...
controller.init=function(){
        ...
        setInterval(function(){
                    if (controller.play){
                          var newTime =$.now();
                          topics.overallTime+=newTime-topics.lastTime;
                          topics.lastTime= newTime;
                    }
                    var seconds = Math.floor(topics.overallTime/1000)%60;
                    var minutes = Math.floor(topics.overallTime/1000/60);
...
controller.calculateTopic=function(pos){
    ...
    var overallDist = 0;
    topicIdx = 0;
    while (topicIdx<topics.topics.length){
        overallDist+=controller.getTopicSpan(topics.topics[topicIdx])+2;
        debug_log2("Controller: overallDist "+overallDist);
        
        if(overallDist>coords.x){
            overallDist-=(controller.getTopicSpan(topics.topics[topicIdx])+1);
            break;
        }
        topicIdx++;
    }
    return {
        topic: topics.topics[topicIdx], 
        dist: coords.x-overallDist+topics.topics[topicIdx].maxNegativeTopicDist
    };
...
controller.mouseDown=function(e){
    ...
    if(pageX-10<Math.min(topics.maxOffsetX*screen.template.elem.width+screen.offsetX+screen.template.x,screen.rightBorder.x)
     ...
     && pageY-screen.header.height()-10<(controller.getCurrentTurn()+2)*screen.template.elem.height+screen.template.y//+screen.offsetY 
...
controller.mouseUp=function(e){
    ...
    if (!controller.btnSaveClicked
        ...
        controller.turnFrom(newT.topic,oldT.topic,oldT.dist,newT.dist,topics.overallTime);  
        ...
        $('#turn').text(controller.getCurrentTurn());
...
controller.getTopics = function(id){
    return topics.topics[id];
}

controller.getTopicsNumber = function(){
    return topics.topics.length;
}
controller.updateTopicsOffsetX=function(){
    var offsetX = 0;
    topics.topics.forEach(function(topic){
        topic.offsetX = offsetX;
        offsetX+= controller.getTopicSpan(topic)+2; 
    });
    topics.maxOffsetX=offsetX;
}
//Turn fucntion
controller.turnCounter=function(){
     var counter = 1;
     topics.topics.forEach(function(topic){
        counter+=topic.turns.length;
    });
   return counter;
 
}

controller.getCurrentTurn=function(){
    return controller.turnCounter();
}

controller.popTurn=function(){
    if(controller.turnCounter()>1){
        //Still a bug in some cases
        topics.preTurn=topics.preTurn.preTopic.turns.pop();
        
    }
    return topics.preTurn;
}
//Topic function
controller.turnFrom=function(topic,preTopic,preTopicDist,topicDist,time){
    topic.turns.push(new Turn(preTopic,preTopicDist,topic,topicDist,time));
    topic.maxNegativeTopicDist=Math.min(topic.maxNegativeTopicDist, topicDist);
    topic.maxPositiveTopicDist=Math.max(topic.maxPositiveTopicDist, topicDist);
    controller.updateTopicsOffsetX();
}
controller.getTopicSpan=function(topic){
    //debug_log("Topic span")
    return topic.maxPositiveTopicDist-topic.maxNegativeTopicDist+1;
}
